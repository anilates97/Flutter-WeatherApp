...
import 'package:flutter_weatherapp_with_riverpod/screens/error_screen.dart';
...
class WeatherGPSScreen extends ConsumerWidget {
  ...
  double lat;
  double long;
  WeatherGPSScreen({Key? key, required this.lat, required this.long})
      : super(key: key);
  ...
  Widget build(BuildContext context, watch) {
    ...
    print("lattttt: " +
        ...
        "long: " +
        location![1].toString());
    final colorProvider = watch(getColorGPSProvider(location!));
    return colorProvider.when(
        data: (weather) {
          MyWeatherPreferences.setCityName(weather.location.name);
          return MaterialApp(
            ...
            home: Scaffold(
              ...
              body: RefreshIndicator(
                onRefresh: () {
                  veriVarmi = true;
                  return context.refresh(getWeatherWithGPSProvider(location!));
                },
                child: Container(
                  decoration: BoxDecoration(
                    image: buildBackWalpaper(weather),
                  ),
                  child: ListView(
                    ...
                    children: [
                      Consumer(
                        builder: (context, watch, index) {
                          final provider =
                              watch(getWeatherWithGPSProvider(location!));
                          return provider.when(
                              data: (weather) {
                                print("weather: " + weather.location.name);
                                veriVarmi = true;
                                String img = "http://" +
